...
class WildRGBD_Multi(MultiViewDataset):
    ...
    def _load_data(self):
        cache_file = osp.join(self.ROOT, f"cached_metadata_wildrgbd_{self.split}.h5")
        rank = dist.get_rank() if dist.is_initialized() else 0

        # ---------- build from filesystem ----------
        if not osp.exists(cache_file) and rank == 0:
            # Load the selected sequences from JSON
            selected_seqs_file = osp.join(self.ROOT, f"selected_seqs_{self.split}.json")
            with open(selected_seqs_file, 'r') as f:
            ...
            for category, sequences in selected_seqs.items():
                for seq_name in sequences:
                    seq_dir = osp.join(self.ROOT, category, seq_name)
                    rgb_dir = osp.join(seq_dir, "rgb")
                    depth_dir = osp.join(seq_dir, "depth")
                    mask_dir = osp.join(seq_dir, "masks")
                    metadata_dir = osp.join(seq_dir, "metadata")

                    rgb_paths = sorted([f for f in os.listdir(rgb_dir) if f.endswith(".jpg")])
                    assert len(rgb_paths) > 0, f"{seq_dir} is empty."
                    num_imgs = len(rgb_paths)

                    cut_off = self.num_views if not self.allow_repeat else max(self.num_views // 3, 3)
                    if num_imgs < cut_off:
                        print(f"[WildRGBD] Skipping {seq_name} (imgs={num_imgs} < cut_off={cut_off})")
                        continue

                    img_ids = list(np.arange(num_imgs, dtype=np.int64))
            ...
            with h5py.File(cache_file, "w") as hf:
                ...
                for i, arr in enumerate(self.scene_img_list):
                    ds[i] = arr
            print(f"[WildRGBD] Cached metadata written to {cache_file}")
        ...
        if osp.exists(cache_file):
            print(f"[WildRGBD] Loading cached metadata from {cache_file}")
    ...
    def _fetch_views(self, idx, resolution, rng, num_views, *args, **kwargs):
        ...
        for v, view_idx in enumerate(image_idxs):
            ...
            metadata_path = rgb_path.replace("rgb", "metadata").replace(".jpg", ".npz")

            rgb_image = imread_cv2(osp.join(scene_dir, "rgb", rgb_path), cv2.IMREAD_COLOR)
            depthmap = cv2.imread(osp.join(scene_dir, "depth", depth_path), cv2.IMREAD_UNCHANGED)
            depthmap = depthmap.astype(np.float32)
            depthmap[~np.isfinite(depthmap)] = 0  # invalid
            mask = cv2.imread(osp.join(scene_dir, "masks", mask_path), cv2.IMREAD_GRAYSCALE)
            metadata = np.load(osp.join(scene_dir, "metadata", metadata_path))

            intrinsics = metadata["intrinsics"].astype(np.float32)
            camera_pose = metadata["pose"].astype(np.float32)
